...
def _linear_attention_state_passing_kernel(
    ...
    if ht_ref is not None:
        ...
        @pl.when(pl.program_id(1) == NUM_BLOCKS_S - 1)
        def _():
            ht_ref[...] = h_scratch[...]


@partial(jax.jit, static_argnames=("N", "output_state", "BLOCK_SIZE_S", "BLOCK_SIZE_V"))
def _linear_attention_state_passing_core(
    ...
    kernel = pl.pallas_call(
        ...
        out_shape=(
            jax.ShapeDtypeStruct(shape=(B, NUM_BLOCKS_S * N, K, V), dtype=k.dtype),
