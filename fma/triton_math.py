...
def sigmoid(
    ...
    if APROXIMATE:
        x = tanh(0.5 * x, output_dtype=tl.float32, APROXIMATE=True)
        x = 0.5 * x + 0.5
    else:
...
@triton.jit
def tanh(x, output_dtype: tl.constexpr = None, APROXIMATE: tl.constexpr = False):
    if output_dtype is None:
        output_dtype = x.dtype
    ...
    if APROXIMATE:
        ...
        x = 2 * sigmoid(2 * x, APROXIMATE=False) - 1
