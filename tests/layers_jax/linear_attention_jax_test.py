...
def test_linear_attention_pallas_vmem_fallback(has_input_state: bool, output_state: bool, monkeypatch) -> None:
    if jax.default_backend() != "tpu":
        pytest.skip("KernelBackend.pallas is only supported on TPU")

    # forces the fused kernel's VMEM budget check in `_linear_attention_pallas` to always fail, so
    # this exercises the state-passing + output-only kernel fallback instead of the single fused
    # kernel; a grouped-head layout and NUM_BLOCKS_S = 3 (768 / 256) also cover the head-group
    # mapping and the multi-block prefix-state chaining in that fallback
    monkeypatch.setattr(Accelerator, "get_vmem_bytes", lambda: 1)

    B, S, K, V = 2, 768, 16, 128
    Nq, Nk, Nv = 4, 2, 1
    N = max(Nq, Nk, Nv)

    tolerance = _TOLERANCES[jnp.float32]

    key_q, key_k, key_v, key_h0, key_dy, key_dht = jax.random.split(jax.random.PRNGKey(0), 6)
    std = 0.01

    q = jax.random.normal(key_q, (B, S, Nq, K), dtype=jnp.float32) * std
    k = jax.random.normal(key_k, (B, S, Nk, K), dtype=jnp.float32) * std
    v = jax.random.normal(key_v, (B, S, Nv, V), dtype=jnp.float32) * std
    h0 = jax.random.normal(key_h0, (B, N, K, V), dtype=jnp.float32) * std if has_input_state else None
    ...
    assert_allclose(np.asarray(y_kernel), np.asarray(y_expected), **tolerance)

    if output_state:
        assert_allclose(np.asarray(ht_kernel), np.asarray(ht_expected), **tolerance)
    else:
        assert ht_kernel is None
        assert ht_expected is None

    dy = jax.random.normal(key_dy, y_kernel.shape, dtype=jnp.float32) * std
    dht = jax.random.normal(key_dht, (B, N, K, V), dtype=jnp.float32) * std if output_state else None

    dq_kernel, dk_kernel, dv_kernel, dh0_kernel = vjp_kernel((dy, dht))
    dq_expected, dk_expected, dv_expected, dh0_expected = vjp_expected((dy, dht))

    assert_allclose(np.asarray(dq_kernel), np.asarray(dq_expected), **tolerance)
    assert_allclose(np.asarray(dk_kernel), np.asarray(dk_expected), **tolerance)
    assert_allclose(np.asarray(dv_kernel), np.asarray(dv_expected), **tolerance)

    if has_input_state:
        assert_allclose(np.asarray(dh0_kernel), np.asarray(dh0_expected), **tolerance)
    else:
        assert dh0_kernel is None
        assert dh0_expected is None
