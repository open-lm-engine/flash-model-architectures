...
@cute_op(
    f"{LIBRARY_NAME}::gru_forward_input_dependent_weights_triton",
    mutates_args={"forget_gate", "reset_gate", "output_update", "output"},
)
def gru_forward_input_dependent_weights_triton(
    input: torch.Tensor,
    weight: torch.Tensor,
    ...
    with torch.device(input.device):
        gru_forward_input_dependent_weights_triton_kernel[GRID](
            ...
            W_ptr=weight,
            W_stride_b=weight.stride(0),
            W_stride_s=weight.stride(1),
            W_stride_n=weight.stride(2),
            xf_ptr=forget_input,
