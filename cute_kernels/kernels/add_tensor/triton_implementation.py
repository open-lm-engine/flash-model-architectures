...
def _add_tensor_triton_kernel(x_ptr, y_ptr, output_ptr, N, BLOCK_SIZE: tl.constexpr):
    ...
    if BLOCK_ID < NUM_BLOCKS - 1:
        _add_tensor_triton(x_ptr=x_ptr, y_ptr=y_ptr, output_ptr=output_ptr, indices=indices, mask=None)
    else:
        _add_tensor_triton(x_ptr=x_ptr, y_ptr=y_ptr, output_ptr=output_ptr, indices=indices, mask=indices < N)
