        ...
        x.scalar_type(), "add_scalar_cuda_kernel", ([&] {
            ...
            for (int i = 0; i < x_chunks.size(); i++) {
                ...
                switch (vector_instruction_width) {
                    case 1:
                        _add_scalar_cuda_kernel<scalar_t, scalar_t>
                            <<<NUM_BLOCKS, BLOCK_SIZE>>>(x_chunk.array, y, output_chunk.array, num_elements);
                        break;
                    case 2:
                        using vector_t = typename DType<scalar_t>::nv_dtype2;
                        _add_scalar_cuda_kernel<scalar_t, vector_t>
                            <<<NUM_BLOCKS, BLOCK_SIZE>>>(x_chunk.array, y, output_chunk.array, num_elements);
                        break;
                    case 4:
                        if constexpr (std::is_same_v<scalar_t, fp32>) {
                            _add_scalar_cuda_kernel<scalar_t, fp32_4>
                                <<<NUM_BLOCKS, BLOCK_SIZE>>>(x_chunk.array, y, output_chunk.array, num_elements);
                        } else {
                            _add_scalar_cuda_kernel<scalar_t, fp32_2>
                                <<<NUM_BLOCKS, BLOCK_SIZE>>>(x_chunk.array, y, output_chunk.array, num_elements);
                        }
                        break;
                    case 8:
